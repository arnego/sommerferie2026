...
2.7. **Verifiser** (automatisert via Chrome): Åpne `index.html` i Chrome, ta screenshot, verifiser at fonter laster, farger stemmer, navigasjon scroller, hamburgermeny fungerer. Test ved mobil- og desktop-bredde.
...
3.7. **Verifiser** (automatisert via Chrome): Ta screenshot, bekreft at kart vises med markører og polylinje. Klikk på markører og verifiser at popups viser stoppinfo.
...
4.5. **Verifiser** (automatisert via Chrome): Klikk på ukekort-headere og ta screenshots for å bekrefte expand/collapse. Verifiser at "i dag"-kort viser korrekt nedtelling. Inspiser at stoppinnhold fra fase 1 er synlig.
...
5.7. **Verifiser** (automatisert via Chrome): Klikk på checkbokser, reload siden, ta screenshot og bekreft at avkrysninger er bevart (localStorage). Test nullstill-knapp og fanebytte.
...
6.8. **Verifiser** (automatisert via Chrome): Scroll til budsjett-seksjonen, ta screenshots av tabeller og kort. Resize vindu og bekreft responsivt layout.
...
All verifisering utføres automatisert av Claude. På Windows brukes **Claude in Chrome**-connector (MCP) for visuell og funksjonell testing direkte i brukerens Chrome-nettleser. Alternativt kan **Claude Preview**-plugin (Python HTTP-server på port 3000) brukes som fallback.

### Verktøyvalg

Claude velger verktøy basert på tilgjengelighet i sesjonen:

| Prioritet | Verktøy | Når |
| --- | --- | --- |
| 1 | **Claude in Chrome** | Windows-sesjon med Chrome åpen. Gir tilgang til ekte nettleser med full interaksjon (klikk, scroll, resize, JavaScript-evaluering, screenshots). |
| 2 | **Claude Preview plugin** | Når Claude in Chrome ikke er tilgjengelig. Starter lokal HTTP-server og bruker preview-verktøyene for screenshots, DOM-inspeksjon og JS-evaluering. |
| 3 | **Statisk kodeanalyse** | Fallback når ingen nettlesertilgang finnes. Grep, Read og manuell gjennomgang av HTML/JS-kode. |

### Verifiseringssteg (automatisert av Claude)
...
**Visuell og funksjonell testing (via Chrome/Preview):**
1. Åpne `index.html` i nettleser og ta screenshot — bekreft at siden rendres korrekt
...
4. Kart: scroll til kart-seksjonen, ta screenshot, bekreft markører og polylinje. Klikk på markør og verifiser popup-innhold
5. Reiseplan: klikk på ukekort-headere, ta screenshots for å bekrefte expand/collapse og innhold
...
8. Responsiv: test ved 375px (mobil), 768px (nettbrett) og 1280px (desktop) — ta screenshots og bekreft at layout tilpasser seg
...
| 7.7 Endelig commit og push | Fullfort | Committet og pushet til GitHub |
