...
**Datamodell — nøkkel på (sted, dato), aldri på stopp-ID:**

Værdata ligger i en flat tabell nøklet `"<lat>,<lng>|<dato>"`, f.eks. `"55.73,9.13|2026-07-30"`. Koordinatene rundes til 2 desimaler (≈ 1,1 km — finere enn Open-Meteos 1–11 km modelloppløsning), så stopp på samme sted deler oppføring og ett API-kall.

Dette er en **korrekthetsgaranti, ikke en optimalisering**. Tidligere var cachen nøklet på `stop.id`, som bare er en posisjonsetikett: da Billund ble splittet i to stopp og fergen flyttet fra id 16 til 17, pekte `weather[16]` fortsatt på fergens data, og 1. august viste **Hirtshals-vær under en Billund-overskrift**. Fordi nøkkelen nå utledes fra stoppets egne koordinater og dato ved *lesetidspunkt*, kan et oppslag bare enten treffe riktig eller bomme (tomt) — aldri treffe feil sted.

**Cache-invalidering — fingeravtrykk, ikke manuell versjonsbump:**
- Payload: `{ schema, fingerprint, fetchedAt, entries }` under den stabile nøkkelen `sommerferie2026-weather`.
- `fingerprint` er utledet av alt som påvirker hvilke data vi trenger: stedsbøtte + ankomst/avreise per stopp, pluss `weatherHours`. **Stopp-ID inngår bevisst ikke** — en ren omnummerering endrer ikke været og skal ikke utløse henting.
- Endres reiseplanen (nye datoer, flyttede koordinater, splittede stopp), spriker fingeravtrykket og cachen forkastes automatisk. Dette erstatter mønsteret med å bumpe nøkkelnavnet manuelt (v1→v5), som fem ganger ble glemt eller oppdaget for sent.
- `schema` fanger formendringer i selve datastrukturen. Gamle nøkler (`-v2`…`-v5`) slettes ved oppstart.
- Fingeravtrykket vurderes mot *forsøk*, ikke *resultat*: det lagres etter en henting uansett utfall. En dato som aldri kan hentes gir derfor én ny henting neste lasting — ikke henting ved hver eneste sidelasting.

**Hentestrategi:**
- Henting skjer ved sideåpning, gruppert **per unikt sted** (16 kall for 17 stopp). Datoene klippes til Open-Meteos tillatte vindu (i dag −90 d … +16 d) allerede når gruppene bygges, så et gruppeintervall aldri blir videre enn API-et godtar.
- Vindusgrensene regnes med **kalenderaritmetikk**, ikke millisekunder: `t + 16*86400000` bommer med én dag over en sommertidsovergang.
- Det materialiseres en oppføring for hver *forespurte* dato uansett hva API-et returnerer, slik at «oppføring finnes, men er tom» og «oppføring mangler» forblir to skilbare tilstander.
- Stopp lenger fram enn vinduet hentes ikke; de viser «Værvarsel kommer nærmere reisen». Mangler data av andre grunner, vises «Værvarsel ikke tilgjengelig» — to ulike tilstander som tidligere ble slått sammen.
- Data byttes ut **helt** ved ny henting (ingen fletting), slik at én `fetchedAt` gjelder alle oppføringer. Et sted som feiler blir tomt — aldri gammelt data merket som ferskt.
- Ved nettverksfeil brukes eventuell eldre cache uavhengig av alder. Ingen feilmelding vises til bruker.

> **Alpine-invariant:** enhver leser av værdata (`getDayEntry`, `hasAnyForecast` m.fl.) må lese `this.weather` **før** en eventuell tidlig `return`. Alpine registrerer avhengigheten først når egenskapen faktisk leses; returnerer en hjelper uten å ha rørt den, oppdateres ikke visningen når hentingen blir ferdig.
...
3. **Sjekklister** — sjekklister fordelt på modus: planlegging viser pakking, campingvogn og før avreise; underveis viser ankomst og avreise. Avkryssinger lagres i localStorage. Nullstill-knapp per liste.
...
| Dato | Endring | Av |
| --- | --- | --- |
| 2026-08-01 | **§2.6 Værvarsel: fiks for at «Værvarsel ikke tilgjengelig» kunne henge igjen i inntil en time.** `fetchWeatherForAllStops()` brukte `Promise.allSettled` per sted og svelget stille alle avslag — dersom *samtlige* steder feilet samtidig (f.eks. et forbigående utfall hos Open-Meteo), returnerte funksjonen et tomt resultat uten å kaste. `loadWeather()` tolket det som en vellykket henting: `this.weather` ble satt til `{}` og det tomme resultatet ble lagret i `localStorage` med `fetchedAt = nå` — altså cachet som «friskt» i en hel `weatherTtlMs`-periode (1 time), og en eventuell god, eldre cache ble overskrevet i samme slag. Alle stopp viste «Værvarsel ikke tilgjengelig» (ikke «kommer nærmere reisen», siden datoene var innenfor prognosevinduet) helt til TTL-en utløp — selv etter at API-et var friskt igjen og siden ble lastet på nytt. Fikset ved at `fetchWeatherForAllStops()` nå kaster når minst ett sted var forespurt, men ingen lyktes; `loadWeather()`s eksisterende catch-blokk håndterer da dette som enhver annen hentefeil — beholder en eventuell gammel cache i stedet for å overskrive den med blanke data, og prøver på nytt ved neste sideåpning uavhengig av TTL. Reprodusert og verifisert med en headless Playwright-test som simulerer total API-utkobling etterfulgt av gjenoppretting. | Claude |
| 2026-07-29 | **§2.6 Værvarsel: strukturell fiks — værcachen nøkles nå på (sted, dato) i stedet for stopp-ID.** Etter fem runder med manuell nøkkelbump (v1→v5) er selve feilmodusen fjernet. (1) **Korrekthet:** `this.weather` er nå en flat tabell nøklet `"<lat>,<lng>|<dato>"` (koordinater rundet til 2 desimaler ≈ 1,1 km, finere enn Open-Meteos oppløsning). Nøkkelen utledes fra stoppets egne koordinater ved lesetidspunkt, så en omnummerering kan bare gi bom (tomt) — aldri treff på feil sted. Alle lesere (`getHourCode`, `getHourTemp`, `getDaily`, `hasAnyForecast`, `weatherCellHtml`) tar nå `stop`-objektet i stedet for `stop.id`. (2) **Invalidering:** manuell versjonsbump erstattet av et `fingerprint` over stedsbøtter, ankomst-/avreisedatoer og `weatherHours`; stopp-ID inngår bevisst ikke. Nøkkelnavnet er nå stabilt (`sommerferie2026-weather`), og `schema`-feltet fanger formendringer. Gamle nøkler `-v2`…`-v5` slettes ved oppstart. Fingeravtrykket lagres etter *forsøk*, ikke *resultat*, så en uhentbar dato ikke kan gi henting ved hver sidelasting. (3) **Henting** grupperes per unikt sted (16 kall for 17 stopp; de to Billund-hotellene deler kall), med datoklipping til API-vinduet allerede ved gruppebygging, og kalenderaritmetikk i stedet for millisekunder (`t + 16*86400000` bommer med én dag over sommertidsovergang). (4) **Visning:** «for langt fram i tid» og «mangler data» er nå to ulike tilstander; tabellen erstattes av en kort forklaring i stedet for en tabell full av streker. (5) Ny Alpine-invariant dokumentert: værlesere må røre `this.weather` før tidlig `return`, ellers registreres ingen avhengighet og cellen blir stående tom etter henting. Verifisert med 30 automatiserte tester (stedsbøtter, regresjon på selve buggen, fingeravtrykk-invalidering, ingen-storm, delvis feil, mock-modus, sommertid). | Claude |
| 2026-07-29 | §2.6 Værvarsel: cache-nøkkel bumpet `sommerferie2026-weather-v4` → `-v5`. Etter forrige økts renummerering av Billund-stoppet (delt i to: stopp 15 LEGOLAND Hotel + stopp 16 The Lodge Billund, og Color Line-fergen flyttet fra id 16 til 17) beholdt enheter med en under-1-time gammel v4-cache fortsatt den gamle id→dato-blobben, der `weather[16]` inneholdt fergens datoer (1.–2. aug) i stedet for The Lodge Billunds (30. juli–1. aug). `getHourCode` fant ingen match for 30. og 31. juli i den gamle blobben og viste strek for de datoene, mens 1. aug feilaktig viste fergens (Hirtshals) værdata under Billund-stoppet. Samme rotårsak og fiks som tidligere v1→v2→v3→v4-bumper: dataformen er uendret, så `loadWeather` kunne ikke skille gammelt fra nytt innhold på egen hånd. Bump til v5 tvinger fram én ny henting på alle enheter med riktig id→stopp-kobling. | Claude |
| 2026-07-14 | **Oppfølging av Sjekklister-endringen.** (1) Seksjonens `id` endret fra `pakkelister` til `sjekklister`; både `<section>` og `sections`-oppføringen er oppdatert, og siden navigasjon (`scrollTo(section.id)`) og scroll-observer (`getElementById(s.id)`) er datadrevet av `sections`-arrayet, følger ankerlenker og aktiv-seksjon-markering automatisk med. Ingen hardkodede `#pakkelister`-anker fantes. (2) Kategori-headerne «Ankomst campingplass» / «Avreise campingplass» er fjernet (satt til tom `category`), overflødige nå som fanene selv heter Ankomst/Avreise. Kategori-header-raden vises kun når `item.category` er sann (`x-show="item.category && (…)"`), så tomme lister får ingen tom header — øvrige lister med kategorier er uendret. | Claude |
| 2026-07-14 | **Pakkelister-seksjonen omdøpt til «Sjekklister»** (seksjonsoverskrift + menyetikett; `id="pakkelister"` beholdt for anker/observer). Den enslige underveis-taben «Daglig sjekkliste» er delt i **to pille-taber: «Ankomst» og «Avreise»**, med sjekkpunktene fordelt på hver sin liste (`daglig`-arrayet erstattet av `ankomst` + `avreise`; tilhørende `checklistTabs`-oppføringer, begge `underveisOnly: true`). To nye ankomst-sjekkpunkter lagt til: **«Skru på gassalarm»** og **«Legg ut gråvannstank»**. localStorage er nøkkelbasert per liste, så gamle `daglig`-avkrysninger ignoreres uten migrering. | Claude |
