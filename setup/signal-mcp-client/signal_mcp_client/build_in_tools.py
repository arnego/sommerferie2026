...
def get_group_config(args, group_name: str | None) -> dict | None:
    """Match a group name against group_configs in config.json."""
    if not group_name:
        return None
    try:
        with open(args.config) as f:
            config = json.load(f)
        for group_config in config.get("group_configs", []):
            contains = group_config.get("group_name_contains", "")
            if contains and contains in group_name:
                logger.debug(f"Matched group_config for group: {group_name}")
                return group_config
    except Exception as e:
        logger.warning(f"Failed to read group_configs from config: {e}")
    return None
...
def get_settings(args, session_id, group_name: str | None = None):
    logger.debug(f"get settings for session: {session_id}, group_name: {group_name}")
    settings = get_default_settings(args)

    # Apply group_config system_prompt if this is a group message
    if group_name:
        group_config = get_group_config(args, group_name)
        if group_config:
            if "system_prompt" in group_config:
                settings["system_prompt"] = group_config["system_prompt"]

    # Session-level overrides (e.g. from /settings) take precedence
