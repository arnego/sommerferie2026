...
async def process_conversation_turn(session_id, args, tools, tool_name_to_session, user_message=None, group_name=None):
    settings = get_settings(args, session_id, group_name=group_name)
    ...
    if tool_used:
        async for item in process_conversation_turn(session_id, args, tools, tool_name_to_session, group_name=group_name):
            yield item
